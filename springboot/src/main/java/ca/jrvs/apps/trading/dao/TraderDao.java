...
public class TraderDao extends JdbcCrudDao<Trader> {
  ...
  public int updateOne(Trader entity) {
    String updateSql = "UPDATE " + getTableName() + " SET first_name=?, last_name=?, "
        + "dob=?, country=?, email=? WHERE " + getIdColumnName() + "=?";
    return jdbcTemplate.update(updateSql, makeUpdateValue(entity));
  }

  private Object[] makeUpdateValue(Trader entity) {
    if (!existsById(entity.getId())) {
      throw new RuntimeException("trader not found:" + entity.getId());
    }
    Object[] values = new Object[]{entity.getFirstName(), entity.getLastName(), entity.getDob(),
